...
Most tests mock `docling_extract.extract_markdown` (and are marked `docling` to
...
def test_extract_markdown_none_when_docling_unavailable(monkeypatch):
    monkeypatch.setattr(docling_extract, "docling_available", lambda: False)
    assert docling_extract.extract_markdown("/whatever.pdf") is None


def test_to_extraction_result_shape():
    res = docling_extract.to_extraction_result("pdf", "/x.pdf", "x", _MD)
    assert res.status == "success"
    assert res.kind == "pdf"
    assert res.text == _MD
    assert res.extractor_detail == {"extractor": "docling", "structured": True, "locators": []}
...
def test_provider_uses_docling_when_available(kind, suffix, make_provider, tmp_path):
    ...
    prov = make_provider()
    with patch.object(docling_extract, "extract_markdown", return_value=_MD):
...
def test_provider_falls_back_when_docling_returns_none(tmp_path):
    ...
    prov = DocsProvider()
    with patch.object(docling_extract, "extract_markdown", return_value=None), \

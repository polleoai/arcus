...
**Sandbox guidance:** local `text`/`pdf`/`docs` need zero egress. Remote
`pdf`/`docs` need egress to the single source host. `html` needs broad web egress
(it drives a browser). `youtube` needs YouTube and possibly NotebookLM.
